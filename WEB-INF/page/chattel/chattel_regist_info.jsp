...
function remove_message(id){
	if(confirm("您确定要解除该条信息吗？")){
		...
		chattelController.deletePledge(id, function(){
			...
			$("#anniu_div_isdeleteisnot"+id).html("<div class='content_div_d4_image1'><span class='content_div_d4_text isdelete'>更新</span></div><div class='content_div_d4_image2'><span class='content_div_d4_text isdelete'>解除</span></div>");
			alert("该动产信息解除成功。");
			$("#select_form").submit();
		});
	}
...
<div id="dialog"  style="display: none">
	...
	<form id="select_form" action="chattel/chattel_regist_info.html" method="post">
	<div id="content_div">
	    <div id="content_div_d2">
			<div class="djbtn">
				<div class="content_div_d2_img" style="cursor:pointer" id="danbi">
	        		<span class="content_div_d1_title_text">
	        			单笔登记
					</span>
				</div>
				<div class="content_div_d2_img2" style="cursor:pointer" id="piliang">
	        		<span class="content_div_d1_title_text">
	        			批量登记
					</span>
				</div>
		...
		<div id="content_div_d1">
			<div id="content_div_d1_title" style="cursor:pointer">历史登记信息&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;更多筛选条件</div>
	    </div>


<div id="content_div_d3" class="displayblock">   

<table width="100%"height="50px" border="0" align="center" class="content_div_d4_table">
  <tr align="center">
    <td align="right" width="15%">担保品：</td>
    <td width="20%">
	<select class="selecty" name="cvarId">
		<option value="">请选择...</option>
		<c:forEach var="co" items="${colls}" varStatus="st">
		<c:choose>
			<c:when test="${co.cvarId==cvarId }">
				<option value="${co.cvarId }" selected="selected">${co.cvarName }</option>
			</c:when>
			<c:otherwise>
    ...
    <td align="right" width="15%"> 借款企业：</td>
    <td width="20%"><input id="finanName" name="finanName" type="text" value="${finanName }"/></td>
    <td width="20%"></td>
  </tr>
  <tr align="center">
    <td align="right" width="10%">监管企业：</td>
    <td>
		<select class="selecty" id="selectSupvId" name="supvId">
			<option value="">请选择...</option>
			<c:forEach var="su" items="${supers}" varStatus="st">
				<c:choose>
					<c:when test="${su.supvId==supvId }">
						<option value="${su.supvId }" selected="selected">${su.supvName }</option>
					</c:when>
					<c:otherwise>
    ...
    <td align="right" width="10%">监管仓库：</td>
    <td>
        <input type="hidden" id="selectWarehouseId1" value="${warehouseId }"/>
		<select class="selecty" id="selectWarehouseId" name="warehouseId">
			<option value="">请选择...</option>
		</select>
    </td>
    <td>
    	<div class="content_div_d2_img1" onclick="submit_select()" style="cursor:pointer">
	       <div class="content_div_tijiao">提 交</div>
		</div>
    </td>
  </tr>
</table>
	...
	<div class="table_div">	
		<table class="content_table">
					<tr>	
						<td width="117px">&nbsp;&nbsp;借款企业：</td>
						<td width="175px"><c:out value="${row.finan_name}"></c:out></td>
						<td width="90px">&nbsp;&nbsp;监管企业：</td>
						<td width="195px"><c:out value="${row.supv_name}"></c:out></td>
					...
					<tr>	
						...
						<td>&nbsp;&nbsp;仓库地址：</td>
						<td ><c:out value="${row.guarantee_warehouse_address}"></c:out></td>
						<td >&nbsp;&nbsp;放贷结束时间：</td>
					...
					<tr>	
						...
						<td >&nbsp;&nbsp;登记时间：</td><td><c:out value="${row.insert_time }"></c:out></td>
					</tr>
	...
	<c:choose>
		<c:when test="${row.is_delete == 0 }">
			<div class="anniu_div" id="anniu_div_isdeleteisnot${row.pledge_id }">
				<div class="content_div_d4_image1" onclick="update_message('${row.pledge_id }')">
        			<span class="content_div_d4_text">更新</span>
        		</div>
				<div class="content_div_d4_image2" onclick="remove_message('${row.pledge_id }')">
		...
		<c:otherwise>
			<div class="anniu_div">
				<div class='content_div_d4_image1'>
					<span class='content_div_d4_text isdelete'>更新</span>
				</div>
				<div class='content_div_d4_image2'>
    ...
    <div id="pagediv">
    <jsp:include page="../common/page1.jsp"></jsp:include>
    </div>
    <div id="default"></div>
</div>
